...
class Text2SemanticDecoder(nn.Module):
    ...
    def infer(
        ...
        for _ in tqdm(range(1500)):
            ...
            if early_stop_num != -1 and (y.shape[1] - prefix_len) > early_stop_num:
                logger.debug("use early stop num:", early_stop_num)
                stop = True

            if torch.argmax(logits, dim=-1)[0] == self.EOS or samples[0, 0] == self.EOS:
                # logger.debug(torch.argmax(logits, dim=-1)[0] == self.EOS, samples[0, 0] == self.EOS)
                stop = True
            if stop:
            ...
            # logger.debug(samples.shape)#[1,1]#第一个1是bs
            # import os
            # os._exit(2333)
    ...
    def infer_panel(
        ...
        x_attn_mask = torch.zeros((x_len, x_len), dtype=torch.bool)
        stop = False
        # logger.debug(1111111,self.num_layers)
        cache = {
        ...
        for idx in tqdm(range(1500), disable=True):
