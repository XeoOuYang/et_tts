...
from inference_cli import change_gpt_weights, change_sovits_weights, get_tts_wav
...
class GPTSoVITSGUI(QMainWindow):
    ...
    def synthesize(self):
        GPT_model_path = self.GPT_model_input.text_str()
        SoVITS_model_path = self.SoVITS_model_input.text_str()
        ref_audio_path = self.ref_audio_input.text_str()
        language_combobox = self.language_combobox.currentText()
        language_combobox = i18n(language_combobox)
        ref_text = self.ref_text_input.text_str()
        language_combobox_02 = self.language_combobox_02.currentText()
        language_combobox_02 = i18n(language_combobox_02)
        target_text = self.target_text_input.text_str()
        output_path = self.output_input.text_str()
